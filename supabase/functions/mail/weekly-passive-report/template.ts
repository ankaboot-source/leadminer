...
export default function buildEmail(
  ...
  ${
      total_reachable > 0
        ? `<li>${i18n.weeklyReport.stats.totalReachable}: <strong>${total_reachable}</strong></li>`
        : ""
    }
  ${
      total_with_phone > 0
        ? `<li>${i18n.weeklyReport.stats.withPhone}: <strong>${total_with_phone}</strong></li>`
        : ""
    }
  ${
      total_with_company > 0
        ? `<li>${i18n.weeklyReport.stats.withCompany}: <strong>${total_with_company}</strong></li>`
        : ""
    }
  ${
      total_with_location > 0
        ? `<li>${i18n.weeklyReport.stats.withLocation}: <strong>${total_with_location}</strong></li>`
        : ""
    }
    </ul>

    <p style="font-size: 16px; margin: 15px 0">
      ${i18n.weeklyReport.viewInDashboard}
    </p>
    ...
    <p style="font-size: 15px; margin: 30px 0 0; color: #374151; font-style: italic;">
      ${i18n.weeklyReport.signature}
    </p>
  `;
...
<html lang="${language}">
  <head>
    ...
    <title>${i18n.weeklyReport.subject}</title>
    <link
            ...
            <tr>
              <td style="text-align: center">
                <h1
                  ...
                  "
                >
                  ${i18n.weeklyReport.headerTitle}
                </h1>
                <p style="margin: 10px 0 0; font-size: 16px; color: #374151">
                  ${i18n.weeklyReport.headerSubtitle}
                </p>
              </td>
            </tr>
            ...
            <tr>
              ...
              >
                <p style="margin: 0 0 4px">
                  ${i18n.weeklyReport.footer.line1}
                  <strong>
                    <a
                      style="color: #6b7280; text-decoration: none"
                      href="${FRONTEND_HOST}"
                    >
                      ${i18n.weeklyReport.footer.brand}
                    </a>
                  </strong>
                  .
                </p>
                <p style="margin: 6px 0 0; color: #9ca3af">
                  ${i18n.weeklyReport.footer.line2}
                </p>
              </td>
            </tr>
          </table>
