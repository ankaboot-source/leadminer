-- location is text[], converting it to text
...
SET location_temp = 
  CASE 
    WHEN location IS NOT NULL AND cardinality(location) > 0 THEN 
      regexp_replace(array_to_string(location, ', '), E'\\n+', ', ', 'g')
    ELSE NULL
  END;
...
-- Update get_contacts_table() `location` input type
