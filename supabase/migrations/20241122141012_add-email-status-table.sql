...
create table
  public.email_status (
    ...
    constraint email_status_pkey primary key (email, user_id)
  );
...
-- Optional: Define a trigger to update the updated_at column
create trigger handle_updated_at_email_status before
update on public.email_status for each row
execute function extensions.moddatetime ('updated_at');

DROP FUNCTION IF EXISTS enrich_contacts;

CREATE FUNCTION "public"."enrich_contacts" (
...
DROP FUNCTION IF EXISTS get_contacts_table;

CREATE FUNCTION "public"."get_contacts_table" ("user_id" "uuid") RETURNS TABLE (
  "source" "text",
  "email" "text",
  "status" "status_enum",
  "name" "text",
...
BEGIN
  RETURN QUERY WITH ExportedContacts AS (
    ...
    INNER JOIN 
      public.email_status es on es.email = p.email 
    LEFT JOIN
...
DROP FUNCTION IF EXISTS get_contacts_table_by_emails;

CREATE FUNCTION "public"."get_contacts_table_by_emails" ("user_id" "uuid", "emails" "text" []) RETURNS TABLE (
  "source" "text",
  "email" "text",
  "status" "status_enum",
  "name" "text",
