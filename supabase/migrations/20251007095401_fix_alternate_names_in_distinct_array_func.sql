...
CREATE OR REPLACE FUNCTION private.refine_persons(userid uuid) RETURNS void
LANGUAGE plpgsql
SET search_path = ''
AS $$
BEGIN
    -- Points of contact for this user
    ...
    -- Combine data with fixed alternate_name
    ...
    SELECT
        ...
        private.get_distinct_or_exclude_from_array(
            pn.alternate_name,
            ARRAY[pn.preferred_name, ea.person_email]  -- Exclude preferred name and email
        ) AS alternate_name
    ...
    DELETE FROM private.messages WHERE user_id = userid;
END;
$$;
