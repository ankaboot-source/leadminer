-- Add `last_error` to the SMS campaign overview RPC so the frontend can
-- surface the reason a campaign ended up in `failed` status (e.g. the
-- worker was killed by the wall-clock watchdog).

DROP FUNCTION IF EXISTS private.get_sms_campaigns_overview();
