...
const renderFooterForCounter = () => {
  const footerTemplate = (form.footerTextTemplate || '').trim();
  if (!footerTemplate) {
    return '';
  }
  return footerTemplate.replaceAll(
    UNSUBSCRIBE_PLACEHOLDER,
    UNSUBSCRIBE_SAMPLE_URL,
  );
};

const updateCharCount = () => {
  const messageText = form.messageTemplate || '';
  const footerText = renderFooterForCounter();
  const finalText = footerText
    ? `${messageText}\n\n${footerText}`
    : messageText;

  const isUnicode = hasUnicodeChars(finalText);
  encoding.value = isUnicode ? 'Unicode' : 'GSM-7';

  const maxPerSms = isUnicode ? 70 : 160;
  smsParts.value = Math.ceil(finalText.length / maxPerSms) || 1;
  charCount.value = finalText.length;
};

const maxCharsPerSms = computed(() =>
  encoding.value === 'Unicode' ? 70 : 160,
);
const charactersLeftLabel = computed(() => {
  const perSms = maxCharsPerSms.value;
  const left = perSms - (charCount.value % perSms);
