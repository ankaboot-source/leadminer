...
function findEmailAddressType( emailAddress, UserName, domainType ) {
    ...
    function getScore( DomainAndUserName ) {
        ...
        while ( i < length ) {
            if (
                UsernameWithoutSpace[ i ] !== domainAndUserName[ i ] && !domainAndUserName.includes( splittedUserName[ 0 ].toLowerCase() ) && !domainAndUserName.includes( splittedUserName?.[ 1 ]?.toLowerCase() )
            ) {
    ...
    if ( UserName.length > 0 ) {
        ...
        if (
            domainType == "provider" && domainType != "custom" && getScore( domainAndUserName[ 0 ] ) > 40
        ) {
            return "Personal";
        }
...
function sortDatabase( dataFromDatabse ) {
    const data = dataFromDatabse.map( ( row ) => {
            ...
            row.dataValues.total = parseInt( row.dataValues.sender ) + parseInt( row.dataValues.recipient );
            row.dataValues.type = "";
            if (
                !row.dataValues.Newsletter && !row.dataValues.Transactional && row.dataValues.name != [ "" ]
            ) {
    ...
    const dataend = wordArr.concat( numArr ),
        sorted = dataend.concat( emptyArr );
