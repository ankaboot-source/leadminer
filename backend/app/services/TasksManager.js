...
class TasksManager {
    ...
    createTask(miningId, userId, fetcher) {
        ...
        if (task === undefined) {
            ...
            this.progressSubscriber.subscribe(miningId, (err) => {
                err && logger.error('Failed subscribing to Redis.', { metadata: { err } });
            });
    ...
    #notifyProgress(miningId, progressType) {
        ...
        if (sseProgressHandler) {
            ...
            switch (progressType) {
                case 'fetching':
                    return sendSSE(sseProgressHandler, parseInt(fetching), `fetching-${miningId}`);
                case 'extracting':
                    return sendSSE(sseProgressHandler, parseInt(extracting), `extracting-${miningId}`);
                default:
            }
        }
    }
