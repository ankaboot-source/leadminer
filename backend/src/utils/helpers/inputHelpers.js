 ...
 *
 * @example if the given number is 9965 then we look to the type param, if it's "position"
 ...
 * so we can stream with the same frequency based on the folder length and position and also mining speed (stream only if there is progress)
...
function EqualPartsForSocket(total, type) {
  ...
  function isInRange(n, nStart, nEnd) {
    return n >= nStart && n <= nEnd;
  }
  let boxCount = total;
  const values = [];
  let n = defaultSocketParts;

  for (const i of casesObject) {
    if (isInRange(boxCount, i[1], i[2])) {
      n = i[0];
      break;
