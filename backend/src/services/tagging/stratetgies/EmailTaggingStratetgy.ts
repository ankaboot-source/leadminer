...
import { isNoReply, isTransactional, isNewsletter } from "../../../utils/helpers/emailAddressHelpers";
...
export class EmailTagging implements TaggingStratetgy {
    ...
    extractTags({ header, emailAddress, emailDomainType, emailFoundIn }: EmailMessageContent) {
        ...
        for (const { rules, name, reachable } of this.tags) {
            ...
            const hasMetCondition = rules.some(
                (condition) => condition.checkCondition({ header, emailAddress, emailDomainType, emailFoundIn })
            )
