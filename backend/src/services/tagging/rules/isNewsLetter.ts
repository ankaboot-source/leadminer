...
export class IdentifyNewsLetterRule implements TaggingCondition {
    checkCondition({ header, emailDomainType, emailAddress, emailFoundIn }: EmailMessageContent) {
      
      const hasRequiredHeaderFields = getSpecificHeader(header, ['list-unsubscribe', 'list-id']) !== null;    
      const hasPrecedenceHeaderField = !!header.precedence

      const hasBulkPriority = hasPrecedenceHeaderField && header.precedence[0] === 'bulk'
