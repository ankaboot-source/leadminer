...
export default class CsvExport implements ExportStrategy<Contact> {
  ...
  async export(
    ...
    const delimiter =
      options?.delimiter ?? CsvExport.getLocalizedCsvSeparator(options?.locale);
    ...
    const content = await CsvExport.getCsvStr(
      [
  ...
  static getLocalizedCsvSeparator(locale?: string) {
