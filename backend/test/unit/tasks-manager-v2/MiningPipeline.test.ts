...
describe('Pipeline', () => {
  ...
  describe('broadcastProgress', () => {
    it('should send SSE event with correct name and current progress value', () => {
      ...
      (manager as any).broadcastProgress('fetched');

      expect(mockSSE.sendSSE).toHaveBeenCalledWith(
        42,
        'fetched-test-mining-id'
      );
    });

    it('should send 0 when progress type has no value', () => {
      const { factory, mockSSE } = makeMockSSEFactory();
      const extract = new ExtractTask({
        miningId: 'test-mining-id',
        userId: 'test-user',
      ...
      expect(mockSSE.sendSSE).toHaveBeenCalledWith(0, 'fetched-test-mining-id');
    });

    it('should use correct event name for extracted progress', () => {
      const { factory, mockSSE } = makeMockSSEFactory();
      const mockFetcher = { startFetch: jest.fn(), stopFetch: jest.fn() };
      ...
      (manager as any).broadcastProgress('extracted');

      expect(mockSSE.sendSSE).toHaveBeenCalledWith(
        30,
        'extracted-test-mining-id'
      );
    });

    it('should send 0 for unknown progress types', () => {
      const { factory, mockSSE } = makeMockSSEFactory();
      const manager = makeManager([], factory);

      (manager as any).broadcastProgress('unknownType');

      expect(mockSSE.sendSSE).toHaveBeenCalledWith(
        0,
        'unknownType-test-mining-id'
      );
    });
  });
