...
describe('regExHelpers.extractNameAndEmail(data)', () => {
  ...
  it('Should return array with one valid object', () => {
    ...
    expect(fullInformationOutput).to.eql([{ name: 'this is myyyyyyyyyyyyyyyy name', identifier: 'tester', address: 'tester@testing.com', domain: 'testing' }]);

    // test only with email
    const emailOnlyOutput = regExHelpers.extractNameAndEmail("<tester@testing.com>");
    expect(emailOnlyOutput).to.eql([{ name: '', identifier: 'tester', address: 'tester@testing.com', domain: 'testing' }]);
