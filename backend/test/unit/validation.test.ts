...
test('rejects a negative number', () => {
  expect(validateType('port', -1, 'number')).toEqual(
    'port must be a valid positive number.'
  );
});

test('rejects 0', () => {
  expect(validateType('port', 0, 'number')).toEqual(
    'port must be a valid positive number.'
  );
});

test('rejects a value that is not a number', () => {
  expect(validateType('port', 'error', 'number')).toEqual(
    'port must be a valid positive number.'
  );
});
...
test('rejects nonboolean values', () => {
  expect(validateType('secure', 'error', 'boolean')).toEqual(
...
test('rejects a nonstring array value', () => {
  expect(validateType('email', [1, 2, 3], 'string[]')).toEqual(
    'email must be an array of non-empty strings.'
  );
});
